...
export default function Stage2({storage}) {
    return (
        <>
            ...
            <div className={styles.choice} onClick={() => recordMbti(storage, 1, 'F')}>
                세상의 모든 이별 노래가 다 내 얘기 같아! 나를 위로해줄 플레이리스트를 찾아본다.
            </div>
            <div className={styles.choice} onClick={() => recordMbti(storage, 2, 'F')}>
                애인보다는 친구가 짱이지! 내 마음을 알아줄 친구를 만나서 실연의 아픔을 털어놓는다.
            </div>
            <div className={styles.choice} onClick={() => recordMbti(storage, 3, 'T')}>
                음, 그렇군. 나 싫다는 사람 굳이 안 붙잡아~ 한번 귀를 후벼파고 금방 잊는다.
            </div>
            <div className={styles.choice} onClick={() => recordMbti(storage, 4, 'T')}>
